...
public class JMCh20Exercises<E>
{
    ...
    public ListNode<E> removeFirst( ListNode<E> head )
    {
        ...
        {
            ListNode<E> fromage = head.getNext();
            head.setNext( null );
            head = fromage;
        }
        return head;
    ...
    public int sizeRecur( ListNode<E> head )
    {
        ...
        return sizeRecur( head.getNext() ) + 1;
    }
    ...
    public ListNode<E> add( ListNode<E> head, E value )
    {
        ...
        lastNode.setNext( new ListNode<E>( value, null ) );
        return head;
    }
    ...
    public ListNode<E> reverseList( ListNode<E> head )
    {
        ...
        ListNode<E> node = head;
        ListNode<E> getNew = null;
        while ( node != null )
        {
            ListNode<E> get = new ListNode<E>( node.getValue(), getNew );
            getNew = get;
            node = node.getNext();
        }
        return getNew;
    ...
    public ListNode<String> concatenateStrings( ListNode<String> head )
    {
        ...
        String getNow = head.getValue();
        ListNode<String> x = head.getNext();
        while ( x != null )
        {
            getNow = getNow + x.getValue();
            x.setValue( getNow );
            x = x.getNext();
        }
        return head;
    ...
    public ListNode<E> getLast( ListNode<E> head )
    {
    ...
    public ListNode<String> insertInOrder( ListNode<String> head, String s )
    {
        ...
        else
        {
            ...
            if ( head.getValue().compareTo( s ) > 0 )
            {
                ListNode<String> newNode = new ListNode<String>( s, head );
                return newNode;
            }
            ListNode<String> node = head.getNext();

            int prev = node.getValue().compareTo( s );
            int after = node.getNext().getValue().compareTo( s );
            while ( node.getNext() != null && node != null )
            {
                ...
                {
                    node.setNext( new ListNode<String>( s, node.getNext() ) );
                    return head;
                }

            }
            if ( node.getNext() == null && node.getValue().compareTo( s ) < 0 )
            {
                node.setNext( new ListNode<String>( s, null ) );
                return head;
            }
            return head;
        }
    ...
    public ListNode<E> middleNode( ListNode<E> head )
    {
        ...
        ListNode<E> med = head;
        ListNode<E> total = head;
        for ( int count = 0; total.getNext() != null; )
        {
            total = total.getNext();
            count++;
            if ( count / 2 == 0 ) // GetMid
     ...
     * Exercise 20.13<br/>
     * 
     * Implement Selection Sort for a linked list with a tail. (Assume that the
     * list holds Comparable objects.) You method should run in O(n<sup>2</sup>)
     * time, where <code>n</code> is the number of nodes in the list.
     * 
     * @param head
     *            reference to a linked list with tail
     * @return sorted list (using Selection Sort)
     */
    public static void sort( ListNode<String> head )
    {
        for ( ListNode<String> one = head; one != null; one = one.getNext() )
        {
            ListNode<String> min = one;
            for ( ListNode<String> two = one; two != null; two = two.getNext() )
            {
                if ( min.getValue().compareTo( two.getValue() ) > 0 )
                {
                    min = two;
                }
            ...
            ListNode<String> node = new ListNode<String>( one.getValue(),
                one.getNext() );
            one.setValue( min.getValue() );
    ...
    {
        ListNode2<Integer> getMax = head;
        ListNode2<Integer> node = head;
        while ( node != null )
        {
            if ( node.getValue() > getMax.getValue() )
            {
                getMax = node;
            }
            node = node.getNext();
        }
        if ( getMax.getNext() == null )
        {
            getMax.getPrevious().setNext( null );
            return head;
        }
        else if ( getMax == head )
        {
            getMax.getNext().setPrevious( null );
            return getMax.getNext();
        }
        getMax.getPrevious().setNext( getMax.getNext() );
        getMax.getNext().setPrevious( getMax.getPrevious() );
        return head;
    }
    ...
    public static void quickSort(
        ListNode2<Double> fromNode,
        ListNode2<Double> toNode )
    {
        // Choose pivot:
            ...
            if ( response.length() > 0 )
            {
                ...
                switch ( response.charAt( 0 ) )
                {
                    ...
                    case '2':
                        ...
                        break;
                    case '3':
                        ListNode<String> head3 = buildList( new String[] { "a",
                            "b", "c", "d" } );
                    ...
                    case '5':
                        ...
                        head5 = buildList( new String[] { "a", "b", "c", "d",
                            "e" } );
                        System.out.print( strList( head5 ) + " = " );
                        System.out.println( ex.sizeRecur( head5 ) );
                        break;
                    ...
                    case '7':
                        ...
                        head7 = buildList( new String[] { "a", "b", "c", "d",
                            "e" } );
                        System.out.println( "Original: " + strList( head7 ) );
                    ...
                    case 'C':
                    case 'c':
                        ListNode<String> headC = buildList( new String[] {
                            "Santa Clara", "San Jose", "San Bruno", "Gilroy" } );
                        System.out.println( "Original: " + strList( headC ) );
                        insHead = ex.moveToBack( headC, "San ?????" );
                        System.out.println( "moveToBack( headC, \"San ?????\" ): "
                            + strList( insHead ) );
                        break;
                    case 'D':
                    case 'd':
                        ListNode<String> headD = buildList( new String[] {
                            "Santa", "bunny", "newt", "apple", "aardvark",
                            "zebra" } );
                    ...
                    case 'E':
                    case 'e':
                        ListNode2<Integer> headE = buildList2( new Integer[] {
                            1, 22, 3, 1234, 99 } );
                    ...
                    case 'F':
                    case 'f':
                        ListNode2<Double> headF = buildList2( new Double[] {
                            101.0, 22.2, 3.22, 1234.0, 99.9 } );
                        ListNode2<Double> tailF = headF.getNext()
                            .getNext()
                            .getNext()
                            .getNext();
