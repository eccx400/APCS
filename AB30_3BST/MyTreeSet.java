...
public class MyTreeSet<E>
{
    ...
    private int width( TreeNode<E> root )
    {
        if ( root != null )
        {
            return max( height( root.getLeft() ) + 1 + height( root.getRight() ),
                max( width( root.getLeft() ), width( root.getRight() ) ) );
        }
        else
        {
            return 0;
    ...
    private int[] nodeCounts( TreeNode<E> root )
    {
        ...
        if ( root != null )
        {
            ...
            result[1] = countNodesHelper( root.getLeft() )
                - countNodesHelper( root.getRight() );
        }
        else
        {
    ...
    private void accumulate( TreeNode<Integer> root )
    {
        ...
        int doge = root.getValue();

        if ( root.getRight() != null )
        {
            doge += root.getRight().getValue();
        }
        if ( root.getLeft() != null )
        {
            doge += root.getLeft().getValue();
        }
        root.setValue( doge );
    }
