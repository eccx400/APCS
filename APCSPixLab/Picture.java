...
public class Picture extends SimplePicture
{
    ...
    public void mirrorVerticalRightToLeft()
    {
        ...
        for ( int row = 0; row < pixels.length; row++ )
        {
            for ( int col = 0; col < wid / 2; col++ )
            {
                leftPixel = pixels[row][col];
                rightPixel = pixels[row][wid - col - 1];
    ...
    public void edgeDetection2( int edgeDist )
    {
        ...
        for ( int row = 0; row < copyPixels.length - 1; row++ )
        {
            ...
            {
                topPix = copyPixels[row][col];
                botPix = copyPixels[row + 1][col];
                botColor = botPix.getColor();
                if ( topPix.colorDistance( botColor ) > edgeDist )
