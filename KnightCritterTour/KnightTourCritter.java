...
{
    /**
     * // An analysis of the accessibility of an 8x8 grid is shown below. // A
     * square with an accessibility of 8 means that it can be // approached from
     * 8 different other squares. A corner square is // rated at 2, while the
     * edges are rated at 3 or 4. It makes sense // to try and visit squares
     * with lower accessibility values first, // leaving the more accessible
     * middle squares for later in the // algorithm.
     */
    ...
    public Location getBestMove( ArrayList<Location> locs )
    {
        ...
        {
            int x = locs.get( i ).getRow();
            int y = locs.get( i ).getCol();
            if ( accessGrid[x][y] > 0 && accessGrid[x][y] < lowestNum)
            {
                bestMove = new Location( x, y );
                lowestNum = accessGrid[x][y];
            }
        }
        return bestMove;
    }
    ...
    {
        ArrayList<Location> fromage = getMoveLocations();

        for ( int count = 0; count < fromage.size(); count++ )
        {
            Location getLocs = fromage.get( count );
            int x = getLocs.getRow();
            int y = getLocs.getCol();
            accessGrid[x][y]--;
        }
        accessGrid[loc.getRow()][loc.getCol()] = -1; // For the negative
    }
    ...
    {
        Location bestLoc = getBestMove( locs );
