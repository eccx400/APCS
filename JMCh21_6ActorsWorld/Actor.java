...
public class Actor implements Comparable<Actor>
{
    ...
    public void readMail()
    {
        ...
        while ( !mailbox.isEmpty() )
        {
            ...
            if ( sub.equals( "need" ) )
            {
                if ( countPossessions( item ) > 1 )
                {
                    reply( sender, "have " + item );
                }
            }
            else if ( sub.equals( "have" ) )
            {
                if ( countPossessions( item ) < 1 )
                {
                    reply( sender, "ship " + item );
                }
            }
            else if ( sub.equals( "ship" ) )
            {
                if ( countPossessions( item ) > 1 )
                {
                    if ( sender.receiveItem( this, item ) )
                    {
    ...
    /**
     * The toString method for this code
     *
     * @return the string stuff
     */
