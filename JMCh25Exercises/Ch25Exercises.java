...
public class Ch25Exercises
{
    ...
    {
        completeTree[0] = (Integer)completeTree[0] / 2;
    }
     ...
     */
    public Object[] toArray( TreeNode<Integer> root, int numNodes )
    {
    ...
    public boolean isHeap( Integer[] items, int numNodes )
    {
        for ( int i = 1; i <= numNodes; i++ )
        {
            ...
            else if ( 2 * i - 1 < numNodes )
            {
                if ( !( items[i] < items[2 * i + 1] ) )
                {
                    return false;
                }
            }
    ...
    {
        int temp;
        for ( int counter = 1; counter < numNodes / 2; )
        {
            if ( items[counter] > items[2 * counter + 1] && items[counter] > items[2 * counter] )
            {
                if ( items[2 * counter + 1] > items[2 * counter] )
                {
                    temp = items[2 * counter];
                    items[2 * counter] = items[counter];
                    items[counter] = temp;
                    counter *= 2;
                }
                else
                {
                    temp = items[2 * counter + 1];
                    items[2 * counter + 1] = items[counter];
                    items[counter] = temp;
                    counter = 2 * counter + 1;
                }
            }
            else if ( items[counter] > items[2 * counter] )
            {
                temp = items[2 * counter];
                items[2 * counter] = items[counter];
                items[counter] = temp;
            ...
            else if ( items[counter] > items[2 * counter + 1] )
            {
                temp = items[2 * counter + 1];
                items[2 * counter + 1] = items[counter];
                items[counter] = temp;
                ...
                switch ( response.charAt( 0 ) )
                {
                    ...
                    case '2':
                        TreeNode<Integer> root = new TreeNode<Integer>( 1,
                            new TreeNode<Integer>( 3, new TreeNode<Integer>( 7,
                                null,
                                null ), new TreeNode<Integer>( 5, null, null ) ),
                            new TreeNode<Integer>( 2, new TreeNode<Integer>( 6,
                                null,
                                null ), new TreeNode<Integer>( 4, null, null ) ) );
